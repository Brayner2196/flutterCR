...
class _AdminPasarelasScreenState extends State<AdminPasarelasScreen> {
  ...
  Future<void> _cargar() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          _pasarelas = list
              .map((e) => PasarelaConfigModel.fromJson(e as Map<String, dynamic>))
              .toList();
        });
      } else {
  ...
  Future<void> _eliminar(PasarelaConfigModel p) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Se eliminará la configuración de ${p.nombre}. Esta acción no se puede deshacer.'),
        actions: [
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text('No hay pasarelas configuradas',
                style: TextStyle(fontSize: 16, color: Colors.grey)),
            const SizedBox(height: 8),
            const Text('Agrega una pasarela para que los residentes\npuedan pagar en línea.',
  ...
  void _mostrarFormulario(BuildContext context, PasarelaConfigModel? existente) {
...
class _PasarelaCard extends StatelessWidget {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _chip(pasarela.activa ? 'Activa' : 'Inactiva',
                              pasarela.activa ? Colors.green : Colors.grey),
                          const SizedBox(width: 6),
                          if (pasarela.sandbox)
                            _chip('Sandbox', Colors.orange),
                          const SizedBox(width: 6),
                          _chip('Prioridad ${pasarela.prioridad}', Colors.blue),
                        ],
                      ),
                    ],
                  ),
                ),
                Switch(value: pasarela.activa, onChanged: (_) => onToggle()),
              ],
            ),
  ...
  Widget _icono(TipoPasarela tipo) {
    final (color, icon) = switch (tipo) {
      TipoPasarela.mercadoPago => (const Color(0xFF009EE3), Icons.payment),
      TipoPasarela.wompi       => (const Color(0xFF00C896), Icons.credit_card),
      TipoPasarela.bold        => (const Color(0xFF1A1A2E), Icons.bolt),
    };
    return Container(
      width: 42, height: 42,
  ...
  Widget _credencial(String label, bool tiene) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Icon(tiene ? Icons.check_circle : Icons.cancel_outlined,
            size: 14, color: tiene ? Colors.green : Colors.grey),
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      final body = {
        ...
        if (_webhookCtrl.text.isNotEmpty) 'webhookSecret': _webhookCtrl.text.trim(),
        'sandbox': _sandbox,
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(isEdit ? 'Editar pasarela' : 'Nueva pasarela',
                  style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),

              // Tipo de pasarela
              if (!isEdit) ...[
                const Text('Pasarela', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                const SizedBox(height: 6),
                DropdownButtonFormField<TipoPasarela>(
                  value: _tipo,
                  decoration: const InputDecoration(border: OutlineInputBorder()),
                  items: TipoPasarela.values.map((t) =>
                      DropdownMenuItem(value: t, child: Text(t.nombreLegible))).toList(),
              ...
              _campo(
                ...
                suffix: IconButton(
                  icon: Icon(_mostrarPrivateKey ? Icons.visibility_off : Icons.visibility),
                  onPressed: () => setState(() => _mostrarPrivateKey = !_mostrarPrivateKey),
                ),
              ),
              const SizedBox(height: 12),
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Modo sandbox',
                            style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                        const SizedBox(height: 2),
                        SwitchListTile(
                          title: Text(_sandbox ? 'Pruebas' : 'Producción',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(
            child: Text(texto, style: const TextStyle(fontSize: 12, color: Colors.blue)),
          ),
        ],
      ),
    );
